...
def get_files_list(path_to_sdfs, 
                   actives_name = 'ligand',
                   inactives_name = 'decoy',
                   sufix = '',
                   sort_func = (lambda x: int(x.split('/')[-1].split('.')[0].split('_')[1]))
                  ):
...
def load_cocrys_molecules_from_dir(list_sdf_files):
    '''Function to load molecules from sdf files using rdkit.
    ...
    for file in list_sdf_files:
        ...
        if mols_dict[pdb_id] is None:
            ...
            # FINAL TRY: Molecule is read from '_from_mol2.sdf' without sanaitize it
            if mols_dict[pdb_id] is None:
...
def get_protId_ligName_dic(list_sdf_files):
